...
export class FoodItemsComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.foodItemsService
                ...
                .subscribe(
                    (res: HttpResponse<IFoodItems[]>) => this.paginateFoodItems(res.body, res.headers),
    ...
    trackId(index: number, item: IFoodItems) {
        return item.id;
    }
    ...
    registerChangeInFoodItems() {
        this.eventSubscriber = this.eventManager.subscribe('foodItemsListModification', response => this.reset());
    }
    ...
    protected paginateFoodItems(data: IFoodItems[], headers: HttpHeaders) {
