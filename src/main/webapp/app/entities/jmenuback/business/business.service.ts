...
export class BusinessService {
    public resourceUrl = SERVER_API_URL + 'jmenuback/api/businesses';
    public resourceSearchUrl = SERVER_API_URL + 'jmenuback/api/_search/businesses';
    ...
    search(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http
            .get<IBusiness[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
    }

    protected convertDateFromClient(business: IBusiness): IBusiness {
        const copy: IBusiness = Object.assign({}, business, {
            signupDate: business.signupDate != null && business.signupDate.isValid() ? business.signupDate.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((business: IBusiness) => {
                business.signupDate = business.signupDate != null ? moment(business.signupDate) : null;
            });
        }
        return res;
    }
