...
describe('Payment e2e test', () => {
    ...
    it('should load Payments', async () => {
        ...
        expect(await paymentComponentsPage.getTitle()).to.eq('ionApp.jmenubackPayment.home.title');
    });
    ...
    it('should load create Payment page', async () => {
        ...
        expect(await paymentUpdatePage.getPageTitle()).to.eq('ionApp.jmenubackPayment.home.createOrEditLabel');
    ...
    it('should create and save Payments', async () => {
        ...
        await paymentComponentsPage.clickOnCreateButton();
        await promise.all([paymentUpdatePage.setAmountInput('5'), paymentUpdatePage.setReasonInput('reason')]);
        expect(await paymentUpdatePage.getAmountInput()).to.eq('5');
        expect(await paymentUpdatePage.getReasonInput()).to.eq('reason');
    ...
    it('should delete last Payment', async () => {
        ...
        expect(await paymentDeleteDialog.getDialogTitle()).to.eq('ionApp.jmenubackPayment.delete.question');
